...
class ResizeNormalize(object):
  ...
  def gaussian_blur(self, img):
    ...
    #img = cv2.GaussianBlur(img, (5, 5), np.random.randint(1, 10))
  ...
  def down_up_sample(self, img):
    img = img.resize((int(self.size[0] / (random.random() * 2 + 1)), int(self.size[1] / (random.random() * 2 + 1))),self.interpolation)
    #img = img.resize((int(self.size[0] / 4), int(self.size[1] / 4)),self.interpolation)
  ...
  def __call__(self, img, is_train):
    ...
    lr.sub_(0.5).div_(0.5)
    hr.sub_(0.5).div_(0.5)
    #torch.multiprocessing.set_start_mothod('spawn')
