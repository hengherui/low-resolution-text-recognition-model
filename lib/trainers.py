...
class BaseTrainer(object):
  def __init__(self, model, SAN, metric, logs_dir, iters=0, best_res=-1, grad_clip=-1, use_cuda=True, loss_weights={}):
  ...
  def train(self, epoch, data_loader, optimizer,sr_optimizer, current_lr=0.0,
    ...
    for i, inputs in enumerate(data_loader):
      ...
      # hr = self.add_mean(hr)
      loss_sr = self.sr_crit(fake_HR, hr)
      batch_size = input_dict['images'].size(0)
      output_dict['losses']['loss_sr'] = loss_sr
      batch_size = input_dict['images'].size(0)
      ...
      losses_rec.update(loss_dict['loss_rec'] * self.loss_weights['loss_rec'])
      losses_sr.update(loss_dict['loss_sr'] * self.loss_weights['loss_sr'])
      ...
      if self.iters % print_freq == 0:
        print('[{}]\t'
              ...
              .format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                      ...
                      losses_rec.val, losses_rec.avg,
                      losses_sr.val, losses_sr.avg
                      ))
      ...
      if self.iters % test_freq == 0:
        ...
        print('start')
        save_checkpoint({
          'state_dict': self.san.module.state_dict(),
          'iters': self.iters,
          'best_res': self.best_res,
        }, is_best, fpath=osp.join(self.logs_dir, 'checkpoint_san.pth.tar'))
        print('finish')
