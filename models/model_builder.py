...
class ModelBuilder(nn.Module):
  ...
  def forward(self, input_dict):
    ...
    x_, hr,  rec_targets, rec_lengths = input_dict['images'], \
                                  input_dict['hr'], \
    ...
    if self.STN_ON:
      ...
      stn_input = F.interpolate(x_, self.tps_inputsize, mode='bilinear', align_corners=True)
      stn_input_hr = F.interpolate(hr, self.tps_inputsize, mode='bilinear', align_corners=True)
      stn_img_feat, ctrl_points = self.stn_head(stn_input)
      stn_img_feat_hr, ctrl_points_hr = self.stn_head(stn_input_hr)
      x, _ = self.tps(x_, ctrl_points)
      hr, _ = self.tps(hr, ctrl_points)
      if not self.training:
        # save for visualization
        return_dict['output']['ctrl_points'] = ctrl_points
        return_dict['output']['rectified_images'] = x

    encoder_feats, x1, x2, x3, x4, x5 = self.encoder(x)
    ...
    return return_dict, x1, x2, x3, x4, x5, hr