...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers):
        ...
        self.maxpool2_ = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        #self.maxpool3 = nn.MaxPool2d(kernel_size=2, stride=(2, 1), padding=(0, 0))
        self.maxpool3_ = nn.MaxPool2d(kernel_size=2, stride=(2, 1), padding=(0, 1))
        #self.maxpool4 = nn.MaxPool2d(kernel_size=2, stride=(2, 1), padding=(0, 1))
        self.conv_1 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn_1 = nn.BatchNorm2d(64)
        self.conv_2 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn_2 = nn.BatchNorm2d(128)
        self.conv_3 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn_3 = nn.BatchNorm2d(256)
        self.conv_4 = nn.Conv2d(512,512, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, input):
        ...
        x = self.maxpool2(x)
        c2 = self.layer1(x)   #64 x 16
        c2 = self.conv_1(c2)
        c2 = self.bn_1(c2)
        c2 = self.relu(c2)

        c3 = self.maxpool2_(c2)
        c3 = self.layer2(c3)  #128 x 8
        c3 = self.conv_2(c3)
        c3 = self.bn_2(c3)
        c3 = self.relu(c3)

        c4 = self.maxpool3_(c3)
        c4 = self.layer3(c4)  #256 * 4
        c4 = self.conv_3(c4)
        c4 = self.bn_3(c4)
        c4 = self.relu(c4)
