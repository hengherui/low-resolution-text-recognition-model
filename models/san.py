    ...
    def __init__(self, in_channels, inter_channels=None, dimension=3, mode='embedded_gaussian',
        ...
        if mode in ['embedded_gaussian', 'dot_product', 'concatenation']:
            ...
            if mode == 'embedded_gaussian':
                self.operation_function = _embedded_gaussian
...
class SAN(nn.Module):
    def __init__(self,conv=common.default_conv):
        ...
        rgb_std = (1.0, 1.0, 1.0)
        self.sub_mean = common.MeanShift(255, rgb_mean, rgb_std)#args.rgb_range
        ...
        self.non_local = Nonlocal_CA(in_feat=n_feats, inter_feat=n_feats // 8, reduction=8, sub_sample=False,
                                     bn_layer=False)
        self.conv1 = conv(512, n_feats, kernel_size=1)
        self.conv2 = conv(64, 3, kernel_size=1)
        self.head = nn.Sequential(*modules_head)
        # self.body = nn.Sequential(*modules_body)
        self.tail = nn.Sequential(*modules_tail)
        self.FPN = CRNN_FPN('resnet18', 3, 1)
    ...
    def forward(self, x1, x2, x3, x4, x5):
        #print(x.shape)
        #x = F.interpolate(x, size=(8, 25), mode='bilinear', align_corners=True)
        #print(x5.shape)
        #x = self.sub_mean(x)
